...
public:
  ...
  **/
  ...
  //! Return Cell's adhesive area.
  inline int ReferenceAdhesiveArea() const {
    return ref_adhesive_area;
  }

  inline int AdhesiveArea() {
...
//   inline int SetLambdaPerimeter (const int per){
//   return lambda_perimeter = per;
//   }
...
private:
  ...
  inline void CleanMoments(void) {
    sum_x = sum_y = sum_xx = sum_xy = sum_yy = area = target_area  = perimeter= target_perimeter = 0;
  }
  // used internally by class CellularPotts
  double AddSiteToMoments(int x,int y, double new_l=-1. );

  // used internally by class CellularPotts
  double RemoveSiteFromMoments(int x,int y, double new_l=-1.);

  inline double AddSiteToMomentsMeasureCell(int x,int y, double new_l=-1 ) {
    ...
    // Dish owner;
    // int x_framed = x+round((center_x-x)/ this->owner->SizeX() ) * this->owner->SizeX();
    // int y_framed = y+round((center_y-y)/ this->owner->SizeY() ) * this->owner->SizeY();
    sum_x+= x;//_framed;
    sum_y+=y;//_framed;
  ...
  inline double RemoveSiteFromMomentsMeasureCell(int x,int y, double new_l=-1) {
  ...
  double GetNewLengthIfXYWereAdded(int x, int y);
