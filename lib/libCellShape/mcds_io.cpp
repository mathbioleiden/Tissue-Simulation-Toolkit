...
void MCDS_io::map_edges(cell::cell* cell_ds){
...
void MCDS_io::map_nodes(cell::cell* cell_ds){
  for (mesh::nodes::node_iterator node_ds = cell_ds->state().shape().nodes().node().begin();
    ...
    if(nodes.find(node_ds->ID()) == nodes.end()){
      ...
      highlow_xy(&lowest_x, &highest_x, &lowest_y, &highest_y, 
      	   node->x, node->y, 
      	   node_ds == cell_ds->state().shape().nodes().node().begin() && cell_ds->ID() == 
           multicellds->cellular_information().cell_populations().cell_population().cell().begin()->ID());
      nodes[node_id] = *node;
    }
  }
}

void MCDS_io::map_faces(cell::cell* cell_ds){
    ...
    if(faces.find(face_ds->ID()) == faces.end()){
      ...
      for(unsigned int face_edge_index = 0; face_edge_index < face_ds->edge_ID().size(); 
        ...
        for (int face_edge_node_index = 0; face_edge_node_index < 2; face_edge_node_index ++){
          ...
          highlow_xy(&face->lowest_x, &face->highest_x, &face->lowest_y, &face->highest_y, 
...
void MCDS_io::map_cellshapes(){
  for (cell::cell_population_individual::cell_iterator cell_ds = 
    ...
    std::cout <<  "CellID from MCDS: " << cell_id << std::endl;
    cell->mcds_obj = &(*cell_ds);
    cells[cell_id] = *cell;
    map_nodes(cell->mcds_obj);
    map_edges(cell->mcds_obj);
    map_faces(cell->mcds_obj);
    std::cout << "FaceIDS_SIZE"<< cell->face_ids.size() << std::endl;
  }
  mapped = true;
}
