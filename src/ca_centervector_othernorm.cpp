...
void **CellularPotts::PlotVectors(Graphics *g){
  ...
  for (int i=1;i<=cell->size()-1;i++){
    ...
    int x1,y1,x2,y2;
    if ((*cell)[i].area>0){
    x1=(int) (((*cell)[i].sum_x)/(double)(*cell)[i].area) %sizex;
    y1=(int) (((*cell)[i].sum_y)/(double)(*cell)[i].area) %sizey;
    if ((*cell)[i].GetBorderNumber()>0){
      if (x1>sizex or x1<0){
        cout << "x1 outside field" << endl;
      }
    x2=x1+(int) 10*((*cell)[i].getVectorActX()/(double)(*cell)[i].GetBorderNumber());
    y2=y1+(int)10* ((*cell)[i].getVectorActY()/(double)(*cell)[i].GetBorderNumber());
  }
    else{ x2=0;y2=0;
    }
    // cout << "add line" << endl;
...
if (sumAct>0){
  vx=vx/((sumAct)/(double) par.max_Act);//vx/((sumDist/c.Area())*(sumAct));
  vy=vy/((sumAct)/(double) par.max_Act);//vy/((sumDist/c.Area())*(sumAct));
}
else {
  vx=0;
  vy=0;
...
void CellularPotts::ComputeActVectorAddSite(int new_x, int new_y, Cell &c, PDE *PDEfield, vector<double> &vector_act){
  ...
  if (sumAct>0){
    ...
    vx=vx/(sumAct/(double) par.max_Act);//vx/((sumDist/(c.Area()+1))*(sumAct/(double) par.max_Act));
    vy=vy/(sumAct/(double) par.max_Act);//vy/((sumDist/(c.Area()+1))*(sumAct/(double) par.max_Act));
