...
def tag_boundary_cardinal_directions(sim, par) -> List[TaggedParticle]:
    ecm = sim.get_state()
...
def stretch_network(
    ...
    stretch_steps: int,
    tagged_particles: List[TaggedParticle],
):
    for i in range(stretch_steps):
        ...
        for k, tp in enumerate(tagged_particles):
            move_adhesion_particles.new_pos[k, :] = ecm.particles.positions[
                tp.index, :
            ] + np.array(tp.direction)
            move_adhesion_particles.par_id[k] = tp.index
            _logger.debug(
                "Request particle %s to move from %s to %s "
                % (
                    tp.index,
                    ecm.particles.positions[tp.index, :],
...
def main() -> None:
    ...
    while instance.reuse_instance():
        ...
        tagged_particles = tag_boundary_cardinal_directions(sim, evo_par)
