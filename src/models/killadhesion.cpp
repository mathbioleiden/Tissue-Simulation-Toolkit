...
TIMESTEP {
    try {
        ...
        if (i == 1000) {
            std::cout << "Removing all adhesions!!!" << std::endl;
            std::cout << "Removing adhesions: ";
            for (int i = 0; i < par.sizex; i++) {
                for (int j = 0; j < par.sizey; j++) {
                    PixelPos pixel(i, j);
                    auto adh_at_pixel = dish->CPM->adhesion_mover.index_.get_adhesions(pixel);
                    for (auto adh : adh_at_pixel) {
                        std::cout << adh.par_id << ' ';
                        interactions.remove_adhesion_particles.par_id.push_back(adh.par_id);
                    }
                }
            }
            std::cout << std::endl;
