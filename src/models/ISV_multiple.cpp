...
void InitPDE(PDE *pde, std::vector<int> somite_positions);
...
INIT
{
    try
    {
            int n = par.number_of_ISV;
            int breedte = 20;
            int lengte = 40;
            int offset_from_boundary = 1;

            std::vector<int> somite_positions;
            std::vector<int> isv_positions;
            float bin_size = par.sizex * 1.0 / ( 2.0 * n );
            for (int k = 0; k <=n; k++){
                somite_positions.push_back(2 * bin_size * k);
            }
            for (int k = 1; k <= n; k++){
                std::cout << bin_size * (1 + 2 * (k - 1)) << '\n';
                isv_positions.push_back(bin_size * (1 + 2 * (k - 1)));
            }
            Grid grid;
            int spin = 1;
            for (auto pos : isv_positions) {
                std::cout << "pos = " << pos << std::endl;
                FillRectangleWithCell(
                    grid, spin,
                    {pos-breedte, par.sizey - offset_from_boundary - lengte},
                    {pos, par.sizey - offset_from_boundary}
                );
                spin += 1;
                FillRectangleWithCell(
                    grid, spin,
                    {pos, par.sizey - offset_from_boundary - lengte - lengte / 2},
                    {pos+breedte, par.sizey - offset_from_boundary - lengte / 2}
                );
                spin += 1;
                FillRectangleWithCell(
                    grid, spin,
                    {pos-breedte, par.sizey-offset_from_boundary - 2*lengte},
                    {pos, par.sizey - offset_from_boundary - lengte}
                );
                spin += 1;
            }
        ...
        if (par.isv_init) { 
            ...
            Grid grid;
            GrowInCellsInRectangle(grid, par.n_init_cells, par.size_init_cells, {1,1}, {par.sizex-1,par.sizey-1});
            CPM->setGrid(grid);
            somite_positions.clear();
        }
            CPM->ConstructInitCells(*this);

            CPM->InitialiseEdgeList();
            PDEfield->InitialiseDiffusionCoefficients(CPM);
            InitPDE(PDEfield, somite_positions);
...
void InitPDE(PDE *pde, std::vector<int> somite_positions) {
    // Setup here would be to init the field at the somites positions 
    // and let it diffuse through some forward euleur kind of thingy.

    for (int x = 0; x < par.sizex; x++){
        for (int y = 0; y < par.sizey; y++){
            pde->setValue(0, x, y, 0.0); // semaphorin
            pde->setValue(1, x, y, 0.0); // vegf
        }
    }
//   for (auto pos : somite_positions) {
//      for (int y = 0; y < par.sizey; y++) {
//        pde->setValue(0, pos, y, par.secr_rate[0]);
//        }
//    }
...
//        for (auto pos : somite_positions) {
//            int y = 100;
////            std::cout << pde->get_PDEvars(0, pos-1, y ) << ','
////                      << pde->get_PDEvars(0, pos,   y ) << ','
////                      << pde->get_PDEvars(0, pos+1, y ) << ','
////                      << '\n';
//        }
//    }
...
//         dish->cell[tipcell].FixPolarity(true); // Turn polarity back on!
        ...
        for (int i = 0; i < par.pde_its; i++) {
            dish->PDEfield->ReactionDiffusion(dish->CPM, 1);
        }
...
void PDE::Secrete(CellularPotts *cpm)
{
    ...
    for (int x = 0; x < sizex; x++)
    {
        for (int y = 0; y < sizey; y++)
        {
            ...
            if (cpm->Sigma(x, y) > 0)
            {
                PDEvars[1][x][y] += par.secr_rate[1] * dt;
            }
            else
            {
                // outside cells
                PDEvars[1][x][y] -= par.decay_rate[1] * dt * PDEvars[0][x][y];
            }
        }
    }
    PROFILE_PRINT
