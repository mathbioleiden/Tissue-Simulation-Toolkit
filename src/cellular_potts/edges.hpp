...
{
public:
   const std::array<int,4> & operator[](unsigned index) const { return edgemap[edgeVector[index]]; }
   std::array<int,4> & operator[](unsigned index) { return edgemap[edgeVector[index]]; }

   void insert(std::array<int,4> edge)
   {
       std::pair<iterator,bool>insertion =edgemap.insert(edge, edgevector.size())
       if (insertion->second){
