...
int CellularPotts::IsingDeltaH(int x, int y, PDE *PDEfield)
{
    ...
    for (i = 1; i <= n_nb; i++)
    {
        ...
        if (neigh_sxy == -1)
        { // border
            cerr << "Only periodic boundaries implemented for Kawasaki "
                    "dynamics sofar.\n";
            exit(1);
                          ...
                          AdhesionDisplacements *adh_disp)
{
    ...
    cout << "DH_perimeter "<< DH_perimeter << '\n';
    DH += DH_perimeter;
    */
...
{ // add an edge to the end of edgelist
...
{ // remove an edge from the edgelist
    ...
    { // if edge is not the last edge in orderedgelist
        // move the edge in the last position to the position of the edge that
        // must be deleted
...
int CellularPotts::CounterEdge(int edge)
{
    ...
    const int counterneighbourlist[20] = {
        3, 4, 1, 2, 7, 8, 5, 6, 11, 12, 9, 10, 17, 18, 19, 20, 13, 14, 15, 16};
    ...
    for (int i = 0; i < sizex; i++)
    {
        for (int j = 0; j < sizey; j++)
            ...
            { // i.e. not medium and not border state (-1)
                ...
                // cell is added (push_back). Then, the array *cell is relocated
                // and the pointer will be lost...

                Cell *motherp = &((*cell)[sigma[i][j]]);
                Cell *daughterp;

                /* Divide if NOT medium and if DIV bit set or divide_always is
                 * set */
                ...
                if (!which_cells.size() || which_cells[motherp->sigma])
                {
                    if (!(divflags[motherp->Sigma()]))
                    {
                        ...
                        // constructor of Cell and in the default copy constr.
                        // of its base class Cytoplasm note: also the celltype
                        // is inherited
                    }
                    else
                    {
                    ...
                    /* if site is below the minor axis of the cell: sigma of new
                     * cell */
                               ...
                               int offset_x, int offset_y)
{
    ...
    for (int i = 0; i < n_cells; i++)
    {
        ...
        if (n_cells > 1)
        {
            middle_x = RandomNumber(sx) + offset_y;
            middle_y = RandomNumber(sy) + offset_y;
        }
        else
        {
            middle_x = sx;
        ...
        int spin = ++cellnum;
        int sq_radius = par.size_init_cells / 3.1415;
        for (int i = 0; i < par.sizex; i++)
            for (int j = 0; j < par.sizey; j++)
                if ((i - middle_x) * (i - middle_x) +
                        (j - middle_y) * (j - middle_y) <=
                    sq_radius)
                    sigma[i][j] = spin;
    }
    ...
    //                        int yp = ny[xyp] + y;
    //                        int kp;
    //                        //  NB removing this border test yields
    //                        interesting
    //                        //  effects :-)
...
bool CellularPotts::LocalConnectedness(int x, int y, int s)
{
    ...
    bool looped = false;
    if (sigma[x + cyc_nx[0]][y + cyc_ny[0]] == s &&
        sigma[x + cyc_nx[7]][y + cyc_ny[7]] == s)
        looped = true;
...
double CellularPotts::Compactness(void)
{
    ...
    for (int y = bounds[0] + 1; y < bounds[1] + 1; y++)
        ...
        // add corner point only if a pixel is present at this location or below
        // it.
        {
            np++;
        }
