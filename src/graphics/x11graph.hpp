...
X11Graphics::X11Graphics(int xsize, int ysize, const char *movie_file)
    : Graphics() {
...
void X11Graphics::EndScene(void) {
  if (par.graphics) {
    ...
    if (shm) { // shared memory extension available?
      XShmPutImage(display, window, windowGC, image, 0, 0, 0, 0, image->width,
                   image->height, 0);
    } else
#endif
      XPutImage(display, window, windowGC, image, 0, 0, 0, 0, image->width,
                image->height);
  }
...
#define NOPVM
#ifndef NOPVM
void X11Graphics::SendScene() { SendPicture(image_data, xfield, yfield); }
...
void X11Graphics::ReceiveScene(int machineindex, int beastindex, int ndish) {
  ...
  for (i = 0; i < sizex; i++)
    for (j = 0; j < sizey; j++) {
      if (pseudoCol8)
        image_data[xpos + i + xdim * sizey * (ypos + j)] =
            picture[j + i * sizex];
      else
        XPutPixel(image, xpos + i, ypos + j,
                  colors[picture[j + i * sizex]].pixel);
    }
...
void X11Graphics::MakeColorMap() {
  ...
  XQueryColors(display, DefaultColormap(display, screen), colors,
               colormap_size);
...
#define KEYBUFSIZE 10
  ...
  if (par.graphics) {
    ...
    if (oldwidth == 0 && oldheight == 0) {
      ...
      unsigned int dui;
      Window dw;
      XGetGeometry(display, window, &dw, &di, &di, &oldwidth, &oldheight, &dui,
                   &dui);
    }
...
void X11Graphics::InitGraphics(int xsize, int ysize) {
  ...
  if (par.graphics) {
    ...
    if (pseudoCol8 &&
        XMatchVisualInfo(display, screen, 8, PseudoColor, &visual_info)) {
      visual = visual_info.visual;
      depth = visual_info.depth;
      if (VERBOSE)
        printf("Got PseudoColor with Depth %d\n", depth);
    } else {
      pseudoCol8 = 0;
    ...
    if (depth == 1)
      color_screen = 0;
    else if (colourclass == GrayScale)
      color_screen = 0;
    else if (colourclass == PseudoColor)
      color_screen = 1;
    else if (colourclass == DirectColor)
      color_screen = 1;
    else if (colourclass == TrueColor)
      color_screen = 1;
    else if (colourclass == StaticColor)
      color_screen = 1;
    else if (colourclass == StaticGray)
      color_screen = 1;
    hint.x = 0;
    hint.y = 0;
    hint.width = hsize;
    ...
    window =
        XCreateWindow(display, DefaultRootWindow(display), hint.x, hint.y,
                      hint.width, hint.height, 5, depth, InputOutput, visual,
                      CWColormap | CWBackPixel | CWBorderPixel, &setattributes);
    XSetStandardProperties(display, window, title, title, None, argv, 0, &hint);
...
int X11Graphics::LineClearP(char direction, int pos, int cropcol) {
  ...
  if (direction == 'x') {
    ...
    snprintf(error, 199, "LineClearP error: %c is an unknown option.",
             direction);
    throw(error);
    exit(1);
  }
...
Coordinate X11Graphics::ReplaceBeast(Coordinate old_size, Coordinate new_size) {
  ...
  XClearWindow(display, window);
  XDrawRectangle(display, window, windowGC, offset.x, offset.y, old_size.x,
                 old_size.y);
  XFlush(display);
  ...
  XGrabPointer(display, window, TRUE, ButtonPressMask | PointerMotionMask,
               GrabModeAsync, GrabModeAsync, window, hand, CurrentTime);
...
void X11Graphics::Resize(void) {
  ...
  // Get the geometry
  XGetGeometry(display, window, &dw, &di, &di, &newwidth, &newheight, &dui,
               &dui);
  printf("%d %d\n", newwidth, newheight);
  ...
  if (
    ...
    if (!shm) {
#endif
      char *new_image_data =
          (char *)malloc(xfield * yfield * sizeof(char) * depth);
      XImage *new_image = XCreateImage(display, visual, depth, ZPixmap, 0,
                                       new_image_data, xfield, yfield, 8, 0);
      // Clear new image
      for (int x = 0; x < xfield; x++) {
        for (int y = 0; y < yfield; y++) {
      ...
      if (!XShmAttach(display, &shminfo)) {
        printf(
            "Warning: shared memory extension is available, but allocation of "
            "\n shared memory segment failed... Reverting to private memory. "
            "\n This is not problematic, but it will be a bit slower.\n");
        shm = 0;
  ...
  if (movie_data) {
    ...
    free(movie_data);
    movie_data =
        (unsigned char *)malloc(xfield * yfield * sizeof(unsigned char));
  }
}
...
void X11Graphics::Write(char *fname, int quality) {
  ...
  png_set_IHDR(png_ptr, info_ptr, xfield, yfield, 8, PNG_COLOR_TYPE_RGB,
  ...
  if (!png_colors) {
    ...
    if ((png_colors = (XColor *)malloc(colormap_size * sizeof(XColor))) ==
        NULL) {
