...
void GLGraphics::Draw() {
  if (rect_pos > 0 ) DrawRects();
  rect_pos = 0;
  rect_col_pos = 0;
  if (point_pos > 0 ) DrawPoints();
  point_pos = 0;
  if (line_pos > 0) DrawLines();
  line_pos = 0;
  line_col_pos = 0;
}
...
void GLGraphics::DrawRects() {
  ...
  glBindBuffer(GL_ARRAY_BUFFER, vertexbuffer);
  glBufferData(GL_ARRAY_BUFFER, sizeof(rect) * rect_pos, rects, GL_DYNAMIC_DRAW);
  glBindBuffer(GL_ARRAY_BUFFER, colbuffer);
  glBufferData(GL_ARRAY_BUFFER, sizeof(color) * rect_col_pos, rect_cols, GL_DYNAMIC_DRAW);
...
void GLGraphics::DrawLines() {
  ...
  glLineWidth(mag/1.5);
  glBindBuffer(GL_ARRAY_BUFFER, vertexbuffer);
  glBufferData(GL_ARRAY_BUFFER, sizeof(line) * line_pos, lines, GL_DYNAMIC_DRAW);
  glBindBuffer(GL_ARRAY_BUFFER, colbuffer);
  glBufferData(GL_ARRAY_BUFFER, sizeof(color) * line_col_pos, line_cols, GL_DYNAMIC_DRAW);
...
void GLGraphics::DrawPoints() {
  ...
  glUniform3fv(uni_size, 1, uniform_size);
  glBindBuffer(GL_ARRAY_BUFFER, vertexbuffer);
  glBufferData(GL_ARRAY_BUFFER, sizeof(point) * point_pos, points, GL_DYNAMIC_DRAW);
  glBindBuffer(GL_ARRAY_BUFFER, colbuffer);
  glBufferData(GL_ARRAY_BUFFER, sizeof(color) * point_pos, point_cols, GL_DYNAMIC_DRAW);
  glDrawArrays(GL_POINTS, 0, point_pos);
}

void GLGraphics::DensityPlot(float * data, int xsize, int ysize, float r, float g, float b) {
  float z = 0;
...
void GLGraphics::contourPlot(float * data, int xsize, int ysize, float r, float g, float b) {
  float z = 0;
  float single_rect[] = {-1, 1,z, 1,1,z, -1,-1,z,  
			 -1,-1,z, 1,1,z, 1,-1,z};
  float cols[] = {r,g,b,1, r,g,b,1, r,g,b,1, 
...
int GLGraphics::SetupShaders(){
  GLuint basic_vert_shader = LoadShader("../src/graphics/shaders/basic.vert", GL_VERTEX_SHADER);
  if (!basic_vert_shader) exit(1);
  GLuint scaled_vert_shader = LoadShader("../src/graphics/shaders/scaled.vert", GL_VERTEX_SHADER);
  if (!scaled_vert_shader) exit(1);
  GLuint geom_shader = LoadShader("../src/graphics/shaders/squares.geom", GL_GEOMETRY_SHADER);
  if (!geom_shader) exit(1);
  GLuint frag_shader = LoadShader("../src/graphics/shaders/basic.frag", GL_FRAGMENT_SHADER);
  if (!frag_shader) exit(1);
  GLuint densityplot_shader = LoadShader("../src/graphics/shaders/densityplot.frag", GL_FRAGMENT_SHADER);
  if (!densityplot_shader) exit(1);
  GLuint intplot_shader = LoadShader("../src/graphics/shaders/intplot.frag", GL_FRAGMENT_SHADER);
  if (!intplot_shader) exit(1);
  GLuint cpm_lineplot_shader = LoadShader("../src/graphics/shaders/cpmlineplot.frag", GL_FRAGMENT_SHADER);
  if (!cpm_lineplot_shader) exit(1);
  GLuint contourplot_shader = LoadShader("../src/graphics/shaders/contourplot.frag", GL_FRAGMENT_SHADER);
  if (!contourplot_shader) exit(1);

  basic_program = glCreateProgram();
  glAttachShader(basic_program, scaled_vert_shader);
  glAttachShader(basic_program, frag_shader);
  if (!LinkProgram(basic_program)) std::cout << "Failed to link basic_program" << std::endl;
  ...
  if (!LinkProgram(squares_program)) std::cout << "Failed to link squares_program" << std::endl;
  ...
  if (!LinkProgram(densityplot_program)) std::cout << "Failed to link densityplot_program" << std::endl;
  ...
  if (!LinkProgram(intplot_program)) std::cout << "Failed to link intplot_program" << std::endl;
  ...
  if (!LinkProgram(cpmlineplot_program)) std::cout << "Failed to link cpmlineplot_program" << std::endl;
  ...
  if (!LinkProgram(contourplot_program)) std::cout << "Failed to link contourplot_program" << std::endl;
