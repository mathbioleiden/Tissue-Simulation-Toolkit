...
bool operator==(AdhesionDisplacements const &lhs,
                AdhesionDisplacements const &rhs) {
...
TEST_CASE("Move some adhesions in various ways", "[adhesion_mover]") {
  ...
  mock_ca.sigma_return_values = {{{3, 6}, 1}, {{4, 6}, 0}, {{5, 6}, 0},
                                 {{6, 6}, 0}, {{3, 5}, 1}, {{4, 5}, 1},
                                 {{5, 5}, 2}, {{6, 5}, 2}, {{3, 4}, 1},
                                 {{4, 4}, 1}, {{5, 4}, 2}, {{6, 4}, 3}};
  ...
  SECTION("random extension and uniform selection") {
    ...
    for (int i = 0; i < n; ++i) {
      double dh = mover.move_dh({4, 5}, {5, 5}, disps);
      auto it =
          std::find(expected.begin(), expected.end(), make_tuple(dh, disps));
