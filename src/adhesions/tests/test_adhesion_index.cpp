...
std::unordered_map<PixelPos, std::vector<AdhesionWithEnvironment>> const &
...
TEST_CASE("Calculate Delta-H for an AttachedBond", "[adhesion_index]") {
  ...
  REQUIRE_THAT(b.move_dh({0.0, 0.0}, {0.0, 2.0}), WithinULP(0.0, 4));

  // 0.5 * 0.1 * (1 - 0.5)^2 = 0.0125
  // 0.5 * 0.1 * (2 - 0.5)^2 = 0.1125
  REQUIRE_THAT(b.move_dh({1.0, 1.0}, {0.0, 1.0}), WithinULP(0.1, 4));

  // bond is linear in stiffness k
  b.bond_type.k = 0.2;
  REQUIRE_THAT(b.move_dh({1.0, 1.0}, {0.0, 1.0}), WithinULP(0.2, 4));
  ...
  REQUIRE_THAT(b.move_dh({12.3, 45.6}, {7.89, 0.123}),
               WithinULP(3494.8529704043532, 4));
}

TEST_CASE("Create an AttachedAngleCst", "[adhesion_index]") {
  AttachedAngleCst c({1.0, 1.0}, {2.0, 2.0}, AngleCstType{10.0 * degrees, 1.5});
...
TEST_CASE("Calculate Delta-H for an AttachedAngleCst", "[adhesion_index]") {
  AttachedAngleCst c({1.0, 1.0}, {2.0, 2.0},
                     AngleCstType{165.0 * degrees, 1.5});
  ...
  REQUIRE_THAT(c.move_dh({0.0, 0.0}, {1.0, -1.0}),
  ...
  REQUIRE_THAT(c.move_dh({2.0, 0.0}, {2.0, 0.125}),
...
TEST_CASE("Regression-test acos domain error", "[adhesion_index]") {
  ...
  AttachedAngleCst c({144.836836830561367, 186.707216898972149},
                     {145.055796016773854, 186.488257712759662},
                     AngleCstType{180.0 * degrees, 1.5});

  auto dh = c.move_dh({144.61787764434888, 186.926176085184636},
  ...
  for (int i = 0; i < 10; ++i) {
    ...
    AttachedAngleCst c({cx, cy}, {cx + d, cy - d},
                       AngleCstType{180.0 * degrees, 1.5});
...
TEST_CASE("Calculate Delta-H for an AdhesionWithEnvironment",
          "[adhesion_index]") {
  ...
  REQUIRE_THAT(a.move_dh({0, -1}), WithinRel(0.1715728752538097, 1e-14));

  a.bonds.emplace_back(ParPos(1.0, 2.0), bt1);

  REQUIRE_THAT(a.move_dh({0, -1}), WithinRel(0.3431457505076194, 1e-14));

  a.angle_csts.emplace_back(ParPos(3.0, 2.0), ParPos(4.0, 2.0),
                            AngleCstType(170.0 * degrees, 0.5));
  REQUIRE_THAT(a.move_dh({0, -1}), WithinRel(0.42881939982263173, 1e-14));

  a.angle_csts.emplace_back(ParPos(1.0, 2.0), ParPos(0.1, 2.1),
                            AngleCstType(180.0 * degrees, 0.3));
  REQUIRE_THAT(a.move_dh({0, -1}), WithinRel(0.4952739475998516, 1e-14));
}
...
TEST_CASE("Build Adhesionindex", "[adhesion_index]") {
  ...
  ecm_boundary.particles[1] =
      Particle(1, ParPos{2.3, 4.5}, ParticleType::adhesion);
  index.rebuild(ecm_boundary);
  ...
  CHECK(((awe.bonds.at(0) == ref_abond_01 && awe.bonds.at(1) == ref_abond_21) ||
  ...
  ecm_boundary.particles[4] =
      Particle(4, ParPos{2.7, 4.1}, ParticleType::adhesion);
  index.rebuild(ecm_boundary);
...
TEST_CASE("Get adhesions for a given pixel", "[adhesion_index]") {
  ...
  ECMBoundaryState ecm_boundary;
  ecm_boundary.particles[0] =
      Particle(0, ParPos{2.3, 4.8}, ParticleType::adhesion);
...
TEST_CASE("Move adhesions from one pixel to another", "[adhesion_index]") {
  ...
  ecm_boundary.particles[0] =
      Particle(0, ParPos{2.3, 4.8}, ParticleType::adhesion);
  ecm_boundary.particles[1] =
      Particle(1, ParPos{2.2, 4.3}, ParticleType::adhesion);
  ecm_boundary.particles[2] =
      Particle(2, ParPos{3.3, 4.0}, ParticleType::adhesion);
  ecm_boundary.particles[3] =
      Particle(3, ParPos{3.8, 4.2}, ParticleType::adhesion);
  index.rebuild(ecm_boundary);
...
TEST_CASE("Remove adhesions from a pixel", "[adhesion_index]") {
  ...
  CHECK_THAT(actions.remove_adhesion_particles.par_id,
