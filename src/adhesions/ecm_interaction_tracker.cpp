...
void ECMInteractionTracker::record_new_particle(
        ParPos pos, double bond_attempt_radius)
{
    update_.add_adhesion_particles.new_pos.push_back(pos);
    update_.add_adhesion_particles.bond_attempt_radius.push_back(bond_attempt_radius);
}
...
void ECMInteractionTracker::reset() {
    update_.clear();
}
