...
Data encode_change_type_in_area(ChangeTypeInArea const & ctia, AutoMemory & mem) {
    ...
    return Data::dict(
            ...
            "from_type", static_cast<int>(ctia.from_type),
            "to_type", static_cast<int>(ctia.to_type));
}

Data encode_add_adhesion_particles(
        AddAdhesionParticles const & aap, AutoMemory & mem) {
    ...
    auto new_pos = Data::grid<double>(
            mem.m1.data(), {aap.new_pos.size(), 2u}, {"i", "xy"});
    ...
    return Data::dict(
            "new_pos", new_pos,
            "bond_attempt_radius", bond_attempt_radius);
}

Data encode_move_adhesion_particles(
        MoveAdhesionParticles const & map, AutoMemory & mem) {
    auto par_id = Data::grid<int32_t>(
            map.par_id.data(), {map.par_id.size()}, {"i"});
    ...
    auto new_pos = Data::grid<double>(
            mem.m2.data(), {map.new_pos.size(), 2u}, {"i", "xy"});
...
std::unordered_map<ParId, Particle> decode_particles(DataConstRef const & data) {
    ...
    for (std::size_t i = 0u; i < par_ids.shape(0u); ++i) {
        result[par_ids(i)] = Particle(
                par_ids(i), {positions(i, 0), positions(i, 1)},
...
std::unordered_map<BondTypeId, BondType> decode_bond_types(DataConstRef const & data) {
...
std::unordered_map<BondId, Bond> decode_bonds(DataConstRef const & data) {
    ...
    for (std::size_t i = 0u; i < bond_ids.shape(0u); ++i)
        result[bond_ids(i)] = Bond(
                particle_groups(i, 0), particle_groups(i, 1),
                static_cast<BondTypeId>(types(i)));

    return result;
}

std::unordered_map<AngleCstTypeId, AngleCstType> decode_angle_cst_types(
...
std::unordered_map<AngleCstId, AngleCst> decode_angle_csts(DataConstRef const & data) {
    ...
    for (std::size_t i = 0u; i < angle_cst_ids.shape(0u); ++i)
        result[angle_cst_ids(i)] = AngleCst(
                particle_groups(i, 0), particle_groups(i, 1), particle_groups(i, 2),
                static_cast<AngleCstTypeId>(types(i)));
...
std::pair<Data, AutoMemory> encode_cell_ecm_interactions(
        CellECMInteractions const & interactions)
{
    AutoMemory mem;

    auto change_type_in_area = encode_change_type_in_area(
            interactions.change_type_in_area, mem);

    auto add_adhesion_particles = encode_add_adhesion_particles(
            interactions.add_adhesion_particles, mem);

    auto move_adhesion_particles = encode_move_adhesion_particles(
            interactions.move_adhesion_particles, mem);

    auto const & rap = interactions.remove_adhesion_particles;
    auto remove_adhesion_particles = Data::dict(
            "par_id", Data::grid<int32_t>(
                rap.par_id.data(), {rap.par_id.size()}, {"i"}));

    auto result = Data::dict(
