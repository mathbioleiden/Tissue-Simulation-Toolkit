...
ParameterFile::ParameterFile(std::string const & filename) {
    ...
    if (!in_file.good())
        throw std::runtime_error(
                "Could not open " + filename + " for reading parameters");
    ...
    while (in_file.good()) {
        ...
        if (mid == line.npos)
            throw std::runtime_error(
                    "Error in " + filename + " on line " + std::to_string(lineno) +
...
template <>
bool ParameterFile::get<bool>(std::string const & name) const {
    auto const & value = entries_.at(name);
    if (value == "false") return false;
    if (value == "true") return true;
    throw std::invalid_argument(
            "When reading parameter " + name + ":" +
            " \"" + value + "\" is not a valid boolean value. Please use"
            " \"true\" or \"false\"");
}


template <>
double ParameterFile::get<double>(std::string const & name) const {
...
template <>
int ParameterFile::get<int>(std::string const & name) const {
    auto const & svalue = entries_.at(name);
    std::size_t idx;
...
template <>
std::vector<double> ParameterFile::get<std::vector<double>>(
        std::string const & name) const
{
    ...
    while (!svalue.empty()) {
        ...
        try {
            ...
            throw std::invalid_argument(
                "When reading parameter " + name + ": Expected a double value but" +
                " got " + svalue);
        }
...
template <>
std::vector<float> ParameterFile::get<std::vector<float>>(
        std::string const & name) const
{
    std::string svalue = entries_.at(name);
    std::size_t idx = 0u;
    ...
    while (!svalue.empty()) {
        ...
        try {
            ...
            throw std::invalid_argument(
                "When reading parameter " + name + ": Expected a float value but" +
                " got " + svalue);
        }
