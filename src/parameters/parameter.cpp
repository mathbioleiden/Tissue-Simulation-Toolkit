...
#define PARAMETER(TYPE, NAME, DEFAULT, DESC) \
    if (file.has(#NAME)) NAME = file.get<TYPE>(#NAME);
#define CONSTRAINT(EXPR, MESSAGE)
#include "parameters.hpp"
#undef CONSTRAINT
#undef PARAMETER
...
#define SECTION(TEXT)
#define PARAMETER(TYPE, NAME, DEFAULT, DESC)
#define CONSTRAINT(EXPR, MESSAGE) if (!(EXPR)) throw std::invalid_argument(MESSAGE);
#include "parameters.hpp"
#undef CONSTRAINT
#undef PARAMETER
#undef SECTION
...
void Parameter::WriteComment(std::ostream & stream, std::string const & text) const {
    stream << "\n";
    stream << "# " << text << "\n";
}

template <>
void Parameter::WritePar<>(
        std::ostream & stream, std::string const & name, bool const & value) const
{
...
template <>
void Parameter::WritePar<>(
        std::ostream & stream, std::string const & name, double const & value) const
{
    std::ios::fmtflags state(stream.flags());
    stream.precision(17);
    stream << name << " = " << value << "\n";
...
template <>
void Parameter::WritePar(
        std::ostream & stream, std::string const & name,
