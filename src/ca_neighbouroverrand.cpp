...
//cout << ", matrix_adhesion = " << ComputeCellMatrixAdhesion( (*cell)[sxy], PDEfield) << "\n";
  ...
  if ( sxyp == MEDIUM ) {
    ...
    (*cell)[sxy].Area() )))));
		...
		DH+= (int)(par.lambda * (1. +2.*(double)((*cell)[sxyp].Area())) -
    ...
    (double)((*cell)[sxyp].Area()))))));
		...
		DH+= (int)(2.*par.lambda * (1. + (double)((*cell)[sxyp].Area()) -
...
if (par.area_constraint_type == 0){
    ...
    DH -= par.lambda_perimeter*( DSQR((*cell)[sxy].Perimeter()-(*cell)[sxy].TargetPerimeter())
		       - DSQR(GetNewPerimeterIfXYWereRemoved(sxy,x,y) -(*cell)[sxy].TargetPerimeter() ) );

  }
  else if ( sxy == MEDIUM ) {

    DH -= par.lambda_perimeter* (  DSQR((*cell)[sxyp].Perimeter()-(*cell)[sxyp].TargetPerimeter())
	...
	if (par.lambda_Act)// && NearbyAdhesionSite(x,y,10,PDEfield))
{
   ...
   int nxp =0,nret =0;
	...
	if( (*cell)[sxyp].AliveP()){
			...
			DH-= par.lambda_Act/par.max_Act * Act_expanding;
	}
  if( (*cell)[sxy].AliveP()){

	   DH+= par.lambda_Act/par.max_Act * Act_retracting;
...
/****** Matrix interaction retraction yield energy ****/
	...
	DH+=par.lambda_matrix * (PDEfield->Sigma(3,x,y)-1)/(par.age_saturation + PDEfield->Sigma(3,x,y));
	}
...
int CellularPotts::AmoebaeMove(PDE *PDEfield)
{
  ...
  for (int i=0;i<loop;i++) {
    ...
    if (kp!=-1) {
      ...
      if ( k  != kp ) {
        if (ConnectivityPreservedPCluster(x,y)){
...
int **CellularPotts::SearchNandPlot(Graphics *g, bool get_neighbours)
{
  ...
  jplus=jplus-sizey+2;
	...
	//colour = sigma[i][j];
      ...
      if ( sigma[i][j] != sigma[iplus][j] )  /* if cellborder */ /* etc. etc. */
	{
	  if (g)
	    g->Point( 1, 2*i+1, 2*j );
	  if (get_neighbours) {
		...
		if (neighbours[sigma[i][j]][q]==EMPTY) {
		  neighbours[sigma[i][j]][q]=sigma[iplus][j];
		  break;
		}
		else
		  if (neighbours[sigma[i][j]][q]==sigma[iplus][j])
		    break;
	    }
	    if (sigma[iplus][j]>0) {
	      for (q=0;q<(int)cell->size();q++)
		if (neighbours[sigma[iplus][j]][q]==EMPTY) {
		  neighbours[sigma[iplus][j]][q]=sigma[i][j];
		  break;
		}
		else
		  if (neighbours[sigma[iplus][j]][q]==sigma[i][j])
		    break;
	    }
	  }
	}
	...
	if (get_neighbours) {
	  if (sigma[i][j]>0) {
	    for (q=0;q<(int)cell->size();q++)
	      if (neighbours[sigma[i][j]][q]==EMPTY) {
		neighbours[sigma[i][j]][q]=sigma[i][jplus];
		break;
	      }
	      else
		if (neighbours[sigma[i][j]][q]==sigma[i][jplus])
		  break;
	  }
	  ...
	  if (sigma[i][j+1]>0) {
	    ...
	    for (q=0;q<(int)cell->size();q++)
	      if (neighbours[sigma[i][jplus]][q]==EMPTY) {
		neighbours[sigma[i][jplus]][q]=sigma[i][j];
		break;
	      }
	      else
		if (neighbours[sigma[i][jplus]][q]==sigma[i][j])
		  break;
	  }
	}
      }
...
for (int x=1;x<sizex-1;x++) {
  for (int y=1;y<sizey-1;y++) {
    if (sigma[x][y] == c.sigma) {
      ...
      vx+=(double) ( (x+round((c.getCenterX()-x)/ this->SizeX()) * this->SizeX()) - c.getCenterX())  * PDEfield->Sigma(2,x,y);
      vy+=(double) ( (y+round((c.getCenterY()-y)/ this->SizeY()) * this->SizeY())-c.getCenterX())*(PDEfield->Sigma(2,x,y));
      sumAct+=PDEfield->Sigma(2,x,y);
      sumDist+=sqrt(pow((double)((x+round(((c.getCenterX())-x)/ this->SizeX()) * this->SizeX())-c.getCenterX()),2)+ pow((double)((y+round(((c.getCenterY())-y)/ this->SizeY()) * this->SizeY())-c.getCenterX()),2));
    }
}}
if (sumAct>0){
  vx=vx/((sumDist/c.Area())*(sumAct/(double) par.max_Act));
  vy=vy/((sumDist/c.Area())*(sumAct/(double) par.max_Act));
}
else {
  vx=0;
  vy=0;
...
void CellularPotts::ComputeActVectorAddSite(int new_x, int new_y, Cell &c, PDE *PDEfield, vector<double> &vector_act){
  ...
  double vx=(double) ( (new_x+round((c.getCenterX()-new_x)/ this->SizeX()) * this->SizeX()) - new_com_x)  * par.max_Act;
  double vy=(double) ( (new_y+round((c.getCenterY()-new_y)/ this->SizeY()) * this->SizeY())- new_com_y)* par.max_Act;
  ...
  for (int x=1;x<sizex-1;x++) {
    for (int y=1;y<sizey-1;y++) {
      if (sigma[x][y] == c.sigma) {
        // if (sigma[x][y]== 1){
        // cout << " x addition " << (double) ( (x+round((c.getCenterX()-x)/ this->SizeX()) * this->SizeX()) - c.getCenterX())  * (PDEfield->Sigma(2,x,y)/(double)par.max_Act) << endl;}
        vx+=(double) ( (x+round((c.getCenterX()-x)/ this->SizeX()) * this->SizeX()) - new_com_x)  * PDEfield->Sigma(2,x,y);
        vy+=(double) ( (y+round((c.getCenterY()-y)/ this->SizeY()) * this->SizeY())- new_com_y)*(PDEfield->Sigma(2,x,y));
        sumAct+=PDEfield->Sigma(2,x,y);
